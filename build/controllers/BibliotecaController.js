...
class BibliotecaController {
    ...
    Buscar(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield database_1.default.query("SELECT `COD_DOCUMENTO`, `ID_AUTORES`, `CATEGORIAS`, `NOMBRE_EDITORIAL`,`img`, `TITULO`, `ANO` FROM `documentos_digitales`INNER JOIN `categorias` ON categorias.ID_CATEGORIAS = documentos_digitales.ID_CATEGORIAS INNER JOIN `editoriales` ON editoriales.ID_EDITORIALES = documentos_digitales.ID_EDITORIALES WHERE `TITULO` like " +
                "'%" +
                [Nombre] +
                "%'" +
                "");
